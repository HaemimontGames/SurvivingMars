...
function Dome_Entrance:AddDirectionTunnel(direction, start_spot, end_spot)
	...
	local weight = start_pos:Dist2D(end_pos)* pathfind[1].terrain / terrain.RoadTileSize() * 8 -- do not use entrances unless necessary
	pf.AddTunnel(self, start_pos, end_pos, weight, 1, direction) -- usable by drones and people only
...
function Dome_Entrance:TraverseTunnel(unit, start_point, end_point, direction)
	...
	local box = unit:GetAttach("ResourceStockpileBox")
	if box then
