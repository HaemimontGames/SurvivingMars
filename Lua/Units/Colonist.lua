...
function OnMsg.DataLoaded()
	...
	for _, trait in ipairs(DataInstances.Trait) do
		if trait.daily_update_func then
			TraitsWithDailyUpdates[1+#TraitsWithDailyUpdates] = trait.name
		end
	end
end
...
function Colonist:DailyUpdate()
	...
	local di_traits = DataInstances.Trait
	...
	if interest ~= "" and fail > 0 then
		...
		if interest == "needMedical" and self.traits.Hypochondriac then
			local trait = DataInstances.Trait.Hypochondriac
			self:ChangeSanity(-trait.param*const.Scale.Stat, trait.name)
		end
	end
	...
	if self.refugee_life_remaining then
		...
		if self.refugee_life_remaining <= 0 then
			...
			if self:Random(100) < DataInstances.Trait.Refugee.param then
...
function Colonist:GetPerformanceReasons()
	...
	if self.traits.Enthusiast and morale >= high then
		amount = DataInstances.Trait.Enthusiast.param
		sn = sign(amount)
	...
	elseif self.traits.Melancholic and morale <  low then
		amount = - DataInstances.Trait.Melancholic.param
		sn = sign(amount)
...
function Colonist:Die(reason)
 ...
 -- for not interupting 'die' command from other commands
	self:PushDestructor(function(self)
		...
		pt = GetPassablePointNearby(pt) or pt
...
function Colonist:ProjectPhoenixEffect()
	...
	if self.city:IsTechResearched("ProjectPhoenix") and not traits.Android and self:Random(100) < TechDef.ProjectPhoenix.param1 then
		...
		local heir = GenerateColonistData(self.city, "Youth", "martianborn", self.gender, self.entity_gender, "no traits")		
		for trait_id, _ in pairs(traits) do
			local trait = DataInstances.Trait[trait_id]
			local category = trait and trait.category or false
...
function Colonist:Arrive()
	...
	assert(rocket)
	local from_rocket = IsKindOf(rocket,"SupplyRocket")
	if from_rocket then
		if HintsEnabled then
	...
	self:PushDestructor(function(self)
		...
		if from_rocket then
			...
			pos = rocket:GetSpotLoc(spot)
			local dir = pos - rocket:GetPos()
			dir = dir:SetZ(0)
			rocket:Attach(self, spot)
			self:SetAnim(1, "disembarkRocket")
			Sleep(self:TimeToAnimEnd())
			self:Detach()
			self:SetPos(pos)
			self:SetState("idle")
			...
			if not self.dome then
				-- pick a nearby pos and goto there if there's no dome to go to
				pos = rocket:PickArrivalPos(pos, dir, 20*guim, 10*guim, 90*60, -90*60)
				self:Goto(pos)
				
				self:SetState("idle")
				
				self.city:AddToLabel("Arrivals", self)
			end
		elseif IsKindOf(self.arriving, "Building") then
			self:SetHolder(self.arriving)
			pos = self.arriving:GetPos()
		end
		if not self.dome then
			while IsValid(self) and self.command ~= "Die" and not self.dome do
				local domes, safety_dome = GetDomesInWalkableDistance(self.city, pos)
				local dome = ChooseDome(self.traits, domes, safety_dome)
				if dome then
					self:SetDome(dome)
					Wakeup(self.command_thread)
					break
				end
				WaitWakeup(5000 + Random(0, 5000) )
			end
			self.city:RemoveFromLabel("Arrivals", self)			
			if not IsValid(self) or self.command == "Die" then
				return
			end
		end
...
function OnMsg.BuildingPlaced(bld)
	local arrivals = bld.city.labels.Arrivals
	if IsKindOf(bld, "Dome") and arrivals then		
		for i = #arrivals, 1, -1 do
			arrivals[i]:SetDome(bld)
			Wakeup(arrivals[i].command_thread)
		end
	end
end
...
function Colonist:SetWorkplace(building, shift)
	...
	if not building and not self.workplace then
		return
	end
	if building == self.workplace and shift == self.workplace_shift then
		return
	end
	
	local refresh = not building and self.workplace or not self.workplace and building
	...
	-- set command
	if not self.workplace and self:IsInWorkCommand() then
...
function Colonist:UpdateWorkplace()
	...
	if workplace then
		...
		or workplace:CheckServicedDome(my_dome) ~= my_dome then
...
function Colonist:Idle()
	...
	if not (self.holder or self:IsValidPos()) then
		assert(self.holder or self:IsValidPos())
		if self.current_dome then
			self.current_dome:RandPlaceColonist(self)
		end
	end
	...
	if self.workplace then
		assert(ValidateBuilding(self.workplace))
		...
		if hour >= (workshift_start - 1) and hour <= (workshift_start + 3) then
			local start_work = IsKindOf(self.workplace, "TrainingBuilding") and self:CanTrain() or self:CanWork()
			if start_work then
				if traits.Fit or self.stat_health >= g_Consts.LowStatLevel then
...
function Colonist:VisitService(service)
	...
	if self:EnterBuilding(service) and IsValid(service) then
		...
		if self.traits.Gamer and service:IsOneOfInterests("interestGaming") then
			local trait_gamer = DataInstances.Trait.Gamer
			self:ChangeSanity( trait_gamer.param * const.Scale.Stat, trait_gamer.name )
		end
...
function Colonist:WorkCycle()
	...
	self:PushDestructor(function(self)
		if IsValid(workplace) then
			if self.traits.Idiot then
				if self:Random(100) < DataInstances.Trait.Idiot.param then
	...
	self:PlayPrg(GetWorkPrg(workplace), g_Consts.WorkingHours * const.HourDuration, workplace)
	self:PopAndCallDestructor()
end
...
function Colonist:CheckForcedWorkplace()
	...
	if ValidateBuilding(workplace) and GameTime() - time < g_Consts.ForcedByUserLockTimeout then
		local dome = self:CheckForcedDome() or self.dome
		if workplace:CheckServicedDome(dome) == dome then
...
function Colonist:SetOutside(outside)
	if outside then
		if self:GetEntity() ~= self.inner_entity then
			return
		end
		local entity = GetOutsideEntity(self.entity_gender, self.race, self.age_trait, self.traits)
		self:ChangeEntity(entity)
		self:AddOutsideNeeds()
	else
		if self:GetEntity() == self.inner_entity then
			return
		end
...
function Colonist:SetSpecialization(specialist, init)
	specialist = specialist ~= "none" and specialist or nil
...
function Colonist:FindEmigrationDome()
	...
	if my_dome then
		if not my_dome.accept_colonists then
			...
			return
		elseif not self:IsHomeless()
		and (my_dome.ui_working or GameTime() - my_dome.ui_working_changed < const.ColonistMaxWaitInDomeTurnedOff) then
			-- could stay only if not homeless and if the Dome is switched ON
			eval = TraitFilterColonist(my_dome.traits_filter, traits)
		end
		workplaces = HasFreeWorkplacesAround(my_dome)
	end
	local is_unemployed = self:CanWork() and not IsValid(self.workplace) and not self.user_forced_workplace
	...
	for _, dome in ipairs(self.city.labels.Dome or empty_table) do
		...
		and dome.accept_colonists and dome.ui_working
		and dome:HasPower() and dome:HasWater() and dome:HasAir() then
			...
			if new_is_walking or LR_transport_available then
				...
				if new_eval >= 0 and dome:HasFreeLivingSpace(is_child) then
					local new_workplaces = HasFreeWorkplacesAround(dome)
	...
	win.GetRolloverText = function(self)
		...
		elseif stat == "Morale" then
			...
			if stat == "Comfort" then
				...
				texts[#texts + 1] = T{4375, "Interests: <em><list></em><newline>", list = table.concat(int_names, ", ")}
			end
			local title = false
			for i = 1, #log, 3 do
				if log[i] > time then
					...
					if reason then
						...
						local trait = DataInstances.Trait[reason]
						local reason_text = 
	...
	for trait_id in pairs(self.traits) do
		...
		if trait and trait.show_in_traits_ui then
			...
			if trait.rare then
				...
				texts[#texts + 1] = T{7551, "<trait>", trait = trait.display_name}
			end
		end
	end
	return texts and table.concat(texts, "<newline>") or T{7552, "No traits"}
end
...
function Colonist:CanInteractWithObject(obj, mode)
	if mode == "assign_to_bld" then
		if IsKindOf(obj, "Building") and ValidateBuilding(obj) then
			return obj:ColonistCanInteract(self)
		end
	end
...
function Colonist:CanBeControlled()
	return self:CanChangeCommand() and self.interaction_mode == "assign_to_bld"
end
-------------------------------------- UI -----------------------------------------------
...
function Colonist:GetPinDescription()
	...
	local traits_text = next(traits) and T{7782, "<newline><nbsp><nbsp><em>Traits:</em> <traits_list>", traits_list = table.concat(traits, ", ")} or ""
...
function Colonist:GetGender()
	return DataInstances.Trait[self.gender] and DataInstances.Trait[self.gender].display_name or T{77, "Unknown"}
end

local ColonistCommands = {
	Abandoned = T{4318, "Abandoned the expedition"},
...
function GenerateColonistData(city, age_trait, martianborn, gender, entity_gender, no_traits)
	city = city or UICity
	-- birthplace and age	
	...
	-- traits
	local traits = no_traits and {} or GenerateTraits({city = city, traits = {Martianborn = (not not martianborn or newborn)}}, age_trait=="Child")
...
function Colonist:Getui_control_hint()
	return GetUnitControlInteractionHint(self, T{4339, "<em>Ctrl + <left_click></em>: Move"} ,T{4340, "No available interaction"})
end
...
function Colonist:UpdateMorale()
	...
	if self.traits and self.traits.Enthusiast and new_value >= high_stat_threshold then
		performance = performance + DataInstances.Trait.Enthusiast.param 
	elseif self.traits and self.traits.Melancholic and new_value < low_stat_threshold then
		performance = performance - DataInstances.Trait.Melancholic.param
	end
