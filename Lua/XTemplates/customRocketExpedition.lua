...
if shortcut == "LeftShoulder" or shortcut == "RightShoulder" then
	...
	local amount = obj:GetProperty("TradeAmount")
	local prop_meta = obj:GetPropertyMetadata("TradeAmount")
	local step = shortcut == "LeftShoulder" and -prop_meta.step or prop_meta.step
	local new_amount = Clamp(amount + step, prop_meta.min, prop_meta.max)
	obj:SetTradeAmount(new_amount)
	return "break"
end
